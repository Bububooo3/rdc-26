...
local jump_power = 60 ----> (For linearvelocity)
...
local jumper: Enemy = {
	...
	move = function(self: Enemy)
		...
		local diff = (self.target.HumanoidRootPart.Position - self.char.HumanoidRootPart.Position).Magnitude
		
		-- Check if we're in attack range		
		if diff <= self.attack_range then
			self.canmove = false
			self:attack()
			return
				
		-- Check if we're in lunging range
		elseif diff <= self.attack_range*10 and diff >= self.attack_range*5 and self.canattack then
			self.canmove = false
			self.canattack = false
			...
			self:animate(self.jumpAnim)

			local direction = (self.target.HumanoidRootPart.Position - self.char.HumanoidRootPart.Position).Unit

			self.char.HumanoidRootPart.AssemblyLinearVelocity = Vector3.new(
				direction.X * (jump_power - 10),
				jump_power,
				direction.Z * (jump_power - 10)
			)

			task.delay(attack_cooldown, function() ----> Oh, task.delay, how I love you...
				self.canattack = true
			end)
