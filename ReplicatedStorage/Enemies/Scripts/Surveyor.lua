...
local attack_cooldown = 25 ----> Die in 60 secs, so @ most they do it twice in their life
local sight_range = 150 ----> This guy is special. Does no damage. Marks players periodically for other enemies to target thru obstructions
...
local surveyor: Enemy = {
	...
	attack = function(self: Enemy)
		...
		local affector = Instance.new("Part")
		affector.shape = Enum.PartType.Ball
		affector.Size = Vector3.new(1,1,1)
		affector.Color = Color3.fromRGB(255, 176, 0)
		affector.Material = Enum.Material.Neon
		affector.CanCollide = false
		affector.Anchored = true
		affector.Parent = workspace
		affector.Transparency = 1
		affector.Position = self.char.HumanoidRootPart.Position

		local cxn = affector.Touched:Connect(function(bp)
			if not(CS:HasTag(bp.Parent, "Player")) then return end
			
			CS:AddTag(bp.Parent, "Tagged")
			
			task.delay(attack_cooldown//1.5, function()
				CS:RemoveTag(bp.Parent, "Tagged")
			end)		
		end)

		for i = 1, 10 do ----> No tweening here
			affector.Size = affector.Size:Lerp(Vector3.new(150, 150, 150), i/10)
			affector.Transparency = math.Lerp(affector.Transparency, .8, i/10)
		...
		-- Cooldown
		task.delay(attack_cooldown//1.5, function() ----> ~16.7 seconds
			for i = 10, 1, -1 do ----> No tweening here
				affector.Size = affector.Size:Lerp(Vector3.new(150, 150, 150), i/10)
				affector.Transparency = math.Lerp(affector.Transparency, .8, i/10)
				task.wait()
			end
